...
echo "$($PSQL "SELECT DISTINCT(major) FROM majors LEFT JOIN students ON majors.major_id = students.major_id WHERE students.student_id IS NULL OR students.first_name ILIKE '%ma%' ORDER BY major ASC;")"
